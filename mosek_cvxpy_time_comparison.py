...
import numpy as np
from inverse_methods import unbalanced_emd_str
import time
...
mosek_times_short = np.zeros(repeats)
cvxpy_times_short = np.zeros(repeats)

mosek_times_long = np.zeros(repeats)
cvxpy_times_long = np.zeros(repeats)

for i in range(repeats):
    start = time.time()
    prob_mosek, sol_mosek, _ = unbalanced_emd_str(leadfield, inverse_short.measurements, connections, lam_short, mu, divergence_matrix)
    mosek_times_short[i] = time.time()-start
    if not mute:
        print(f"Mosek took {mosek_times_short[i]:.1f} seconds.")
    start = time.time()
    prob_cvxpy, sol_cvxpy, _ = unbalanced_emd_str(leadfield, inverse_short.measurements, connections, lam_short, mu, divergence_matrix, standard_solver=True)
    cvxpy_times_short[i] = time.time()-start
    if not mute:
        print(f"Cvxpy took {cvxpy_times_short[i]:.1f} seconds.")
        print(f"Short problem relative error: {np.sum(np.abs(sol_mosek.value-sol_cvxpy.value))}")
        print(f"Mosek cost: {prob_mosek.value}, Cvxpy cost: {prob_cvxpy.value}")
    start = time.time()
    prob_mosek, sol_mosek, _ = unbalanced_emd_str(leadfield, inverse_long.measurements, connections_long, lam_long, mu, divergence_long)
    mosek_times_long[i] = time.time()-start
    if not mute:
        print(f"Mosek took {mosek_times_long[i]:.1f} seconds.")
    start = time.time()
    prob_cvxpy, sol_cvxpy, _ = unbalanced_emd_str(leadfield, inverse_long.measurements, connections_long, lam_long, mu, divergence_long, standard_solver=True)
    cvxpy_times_long[i] = time.time()-start
    if not mute:
        print(f"Cvxpy took {cvxpy_times_long[i]:.1f} seconds.")
        print(f"Long problem relative error: {np.sum(np.abs(sol_mosek.value-sol_cvxpy.value))}")
        print(f"Mosek cost: {prob_mosek.value}, Cvxpy cost: {prob_cvxpy.value}")

with open(f"inverse_code/inversion_methods/small_example/lambda_experiments/time_{str(n_sources)}_{repeats}.pkl", "wb") as file:
    pickle.dump([mosek_times_short.tolist(), cvxpy_times_short.tolist(), mosek_times_long.tolist(), cvxpy_times_long.tolist()], file)