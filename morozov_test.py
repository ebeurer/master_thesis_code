...
n_sources = 5000
problem_name = "short_range_problem.pkl"
...
lambdas = np.zeros((len(noise_levels), inverse_problem.measurements.shape[1]))
for i in tqdm(range(len(noise_levels))):
    inverse_problem.add_noise(noise_levels[i], seed)
    for j in tqdm(range(inverse_problem.measurements.shape[1])):
        try:
            lambdas[i,j] = mce_discrepancy_lambda(leadfield, inverse_problem.measurements[:,j], inverse_problem.noise_variance).root
        except:
            lambdas[i,j] = np.inf

print(f"{np.sum(np.isinf(lambdas))} errors...")
with open(f"inverse_code/inversion_methods/small_example/lambda_experiments/short_range_{str(n_sources)}_{seed}.pkl", "wb") as file:
